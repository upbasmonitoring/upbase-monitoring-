...
const ErrorPage = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-background font-sans selection:bg-primary/10 relative overflow-hidden p-8 text-center">
            {/* --- 🌊 BACKGROUND AMBIANCE --- */}
            <div className="absolute inset-0 bg-grid-white/[0.02] dark:bg-grid-white/[0.02] bg-[size:40px_40px]" />
            ...
            <div className="relative z-10 max-w-xl w-full bg-card rounded-[40px] border border-border shadow-[0_40px_100px_-20px_rgba(0,0,0,0.3)] p-12 lg:p-16 group transition-all">
                
                {/* --- 🚀 BRANDING --- */}
                <div className="flex justify-center mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
                    <Logo variant="vibrant" size="lg" />
                </div>

                {/* --- 🚨 ERROR STATUS --- */}
                <div className="mx-auto w-24 h-24 rounded-[32px] bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-10 relative shadow-sm group-hover:scale-110 transition-transform duration-500">
                    <div className="absolute inset-0 rounded-[32px] bg-red-500/10 animate-ping opacity-20" />
                    <AlertCircle className="h-10 w-10 text-red-500" />
                    <div className="absolute -bottom-2 -right-2 h-9 w-9 bg-card border border-border rounded-2xl flex items-center justify-center shadow-lg">
                         <ShieldAlert className="h-4.5 w-4.5 text-red-400" />
                    </div>
                </div>

                <div className="space-y-4 mb-12">
                    <h1 className="text-4xl font-bold tracking-tighter uppercase text-foreground leading-none">
                        System <span className="text-red-500">Interruption</span>
                    </h1>
                    <p className="text-sm font-semibold text-muted-foreground uppercase tracking-widest leading-relaxed">
                ...
                {/* --- 📟 TELEMETRY LOG --- */}
                <div className="bg-secondary/50 rounded-[28px] p-8 mb-12 border border-border text-left relative overflow-hidden group/log">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <Terminal className="h-3.5 w-3.5 text-muted-foreground/30" />
                    ...
                    <code className="text-xs font-bold font-mono text-muted-foreground block truncate uppercase tracking-tight">
                ...
                <div className="flex flex-col sm:flex-row gap-5">
                    ...
                    <Button 
                        variant="ghost"
                        onClick={() => navigate("/")} 
                        className="flex-1 h-14 rounded-2xl border-border bg-secondary text-foreground font-bold uppercase tracking-widest text-[11px] hover:bg-secondary/80 transition-all border"
                    >
                        <Home className="h-4.5 w-4.5 mr-3 text-muted-foreground/40" /> Exit Terminal
                    </Button>
                </div>

                <button 
                  onClick={() => navigate(-1)}
                  className="mt-12 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/40 hover:text-primary transition-all flex items-center justify-center mx-auto gap-3 group"
                >
