        ...
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-6 text-center">
                ...
                <div className="space-y-2">
                    <h2 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Security Protocol Required</h2>
                    <p className="text-xs font-medium text-slate-400 uppercase tracking-widest">Please authenticate to access the Incident Control Center.</p>
                </div>
                <button 
            ...
            <div className="space-y-12 pb-20 font-sans">
                ...
                <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
                    <div className="space-y-3">
                        ...
                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest max-w-lg leading-relaxed">
                    ...
                    <div className="flex items-center gap-8 px-10 py-6 bg-white border border-slate-100 rounded-[32px] shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] group hover:border-primary/20 transition-all">
                         <div className="flex flex-col">
                            <span className="text-[9px] font-bold uppercase tracking-widest text-slate-300 leading-none mb-2">Active Signal Drop</span>
                            <span className={`text-3xl font-bold tracking-tighter tabular-nums leading-none uppercase ${activeAlerts?.length > 0 ? 'text-red-500' : 'text-slate-900'}`}>
                                {activeAlerts?.length || 0} SEVERE
                            </span>
                         </div>
                         <div className="h-10 w-px bg-slate-50" />
                         <div className="h-12 w-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-primary/5 transition-all">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-6 min-h-[400px]">
                            {activeAlerts?.length > 0 ? activeAlerts.map((alert: any) => (
                                <div key={alert._id} className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] hover:border-red-500/20 transition-all group relative overflow-hidden">
                                    ...
                                    <div className="flex items-start justify-between mb-8">
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-3">
                                                <h3 className="text-xl font-bold uppercase tracking-tighter text-slate-900 group-hover:text-red-500 transition-colors">{alert.monitor?.name}</h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-10 pt-8 border-t border-slate-50">
                                        <div className="space-y-1.5">
                                            <p className="text-[9px] font-bold uppercase tracking-widest text-slate-300 flex items-center gap-2 leading-none mb-1">
                                                <Clock className="h-3.5 w-3.5 text-slate-200" />
                                                Incident Duration
                                            </p>
                                            <p className="text-sm font-bold uppercase tracking-tighter tabular-nums text-slate-900">
                                                {formatDuration(alert.monitor?.failureStartedAt)}
                                            </p>
                                        </div>
                                        <div className="space-y-1.5">
                                            <p className="text-[9px] font-bold uppercase tracking-widest text-slate-300 flex items-center gap-2 leading-none mb-1">
                                                <Satellite className="h-3.5 w-3.5 text-slate-200" />
                                                Uplink Status
                                            </p>
                                            <p className={`text-sm font-bold uppercase tracking-tighter ${
                                                alert.type === 'DEGRADED' ? 'text-amber-600' : 'text-red-600'
                                            }`}>{alert.type === 'DEGRADED' ? 'Degraded Node' : 'Node Down'}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="mt-8 pt-8 border-t border-slate-50">
                                        <button className="w-full h-14 bg-slate-50 hover:bg-red-50 border border-slate-100 hover:border-red-100 text-slate-400 hover:text-red-500 text-[10px] font-bold uppercase tracking-widest rounded-2xl transition-all shadow-sm">
                                            Initiate Recovery Handshake
                                        </button>
                                    </div>
                                </div>
                            )) : (
                                <div className="h-[450px] border-2 border-dashed border-slate-100 rounded-[50px] bg-white flex flex-col items-center justify-center text-center gap-10 group hover:border-primary/20 transition-all px-12">
                                    <div className="h-24 w-24 rounded-3xl bg-slate-50 border border-slate-100 flex items-center justify-center group-hover:bg-primary/5 group-hover:text-primary transition-all text-slate-200 shadow-sm relative">
                                    ...
                                    <div className="space-y-4">
                                        <p className="text-[11px] font-bold uppercase tracking-[.5em] text-slate-400">Alert Engine Pending</p>
                                        <div className="space-y-3">
                                            <p className="text-[10px] font-bold uppercase text-slate-300 tracking-widest max-w-[320px] leading-relaxed mx-auto">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-white rounded-[40px] border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] h-[680px] flex flex-col overflow-hidden">
                            <div className="h-18 px-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                                 <span className="text-[10px] font-bold uppercase tracking-widest text-slate-300">Handshake Integrity: Verified</span>
                                 <div className="flex items-center gap-2">
                                     <div className="h-1.5 w-1.5 rounded-full bg-slate-200" />
                                     <span className="text-[9px] font-bold uppercase tracking-widest text-slate-200">Past 20 Events</span>
                                 </div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-10 space-y-5 custom-scrollbar">
                                {alertHistory?.map((log: any) => (
                                    <div key={log._id} className="flex gap-6 p-6 rounded-[32px] bg-white border border-slate-100 hover:border-primary/20 hover:shadow-sm transition-all group/item">
                                        <div className={`h-12 w-12 shrink-0 rounded-2xl flex items-center justify-center border transition-all duration-500 ${
                                            log.type === 'DOWN' ? 'bg-red-50 border-red-100 text-red-500' : 
                                            log.type === 'DEGRADED' || log.severity === 'warning' ? 'bg-amber-50 border-amber-100 text-amber-500' : 
                                            log.type === 'RECOVERY' || log.type === 'UP' ? 'bg-emerald-50 border-emerald-100 text-emerald-500' :
                                        ...
                                        <div className="flex-1 min-w-0 space-y-2">
                                            <div className="flex items-center justify-between gap-4">
                                                <span className="text-sm font-bold uppercase text-slate-900 truncate group-hover/item:text-primary transition-colors">{log.monitor?.name || 'System Alert'}</span>
                                                <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest tabular-nums">{new Date(log.triggeredAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                            </div>
                                            <p className="text-[11px] font-medium text-slate-400 line-clamp-1 uppercase tracking-tight">{log.message}</p>
                                            <div className="flex items-center gap-4 pt-1">
                                                ...
                                                <span className="text-[9px] font-bold text-slate-200 uppercase tracking-widest">{log.severity?.toUpperCase()}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="h-14 border-t border-slate-50 bg-slate-50/30 flex items-center justify-center">
                                 <span className="text-[9px] font-bold uppercase tracking-[.5em] text-slate-200">Terminal Output Finalized</span>
                            </div>
                        </div>
                    </div>
