            ...
            <div className="space-y-12 pb-20 font-sans relative">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-10">
                    <div className="space-y-3 lg:max-w-md">
                        ...
                        <p className="text-xs sm:text-sm font-semibold text-muted-foreground/60 uppercase tracking-widest leading-relaxed">
                            Audit stream for code pushes and production environment transitions. 
                            Automatic impact analysis for every rollout.
                        </p>
                    </div>

                    <div className="flex items-center gap-4 sm:gap-5 p-5 sm:px-10 sm:py-6 bg-card border border-border rounded-2xl sm:rounded-[32px] shadow-[0_15px_40px_-15px_rgba(0,0,0,0.05)] group hover:border-primary/20 transition-all">
                ...
                {selectedImpactId && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-end font-sans">
                        ...
                        <div className="relative w-full max-w-2xl h-full bg-card shadow-[-50px_0_100px_rgba(0,0,0,0.5)] p-0 flex flex-col animate-in slide-in-from-right duration-500 border-l border-border text-foreground">
                            ...
                            <div className="p-6 sm:p-10 flex items-center justify-between border-b border-border bg-secondary/20">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-4 sm:gap-5">
                                        <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl sm:rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center">
                                            <AlertTriangle className="h-5 w-5 sm:h-6 sm:w-6 text-red-500" />
                                        </div>
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-bold uppercase tracking-tighter text-foreground leading-none">
                                                Impact <span className="text-red-500">Analysis</span>
                                            </h2>
                                            <p className="text-[9px] sm:text-[10px] font-bold text-muted-foreground/40 uppercase tracking-[.4em] mt-1.5 whitespace-nowrap">Incident Telemetry Log</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            {/* Drawer Content */}
                            <div className="flex-1 overflow-y-auto p-6 sm:p-12 space-y-10 sm:space-y-14 custom-scrollbar">
                                {/* Impact Status Hub */}
                                <div className="p-6 sm:p-10 rounded-2xl sm:rounded-[40px] bg-red-500/5 border border-red-500/10 space-y-6 sm:space-y-8 relative overflow-hidden group">
                                    ...
                                    <div className="flex items-start gap-4 sm:gap-8 relative z-10">
                                        <div className="h-12 w-12 sm:h-16 sm:w-16 rounded-xl sm:rounded-2xl bg-card border border-red-500/20 flex items-center justify-center shrink-0 shadow-sm">
                                            <Target className="h-6 w-6 sm:h-8 sm:w-8 text-red-500" />
                                        </div>
                                        <div className="space-y-2 pt-1">
                                            <h3 className="text-base sm:text-lg font-bold uppercase tracking-widest text-red-500">Performance Regression</h3>
                                            <p className="text-[10px] sm:text-sm font-semibold text-muted-foreground uppercase tracking-tight leading-relaxed">Direct correlation between Commit #{impactData?.commitId?.slice(0,7)} and node instability.</p>
                                        </div>
                                    </div>
                                ...
                                {impactData?.incident?.aiRca && (
                                    <div className="space-y-5">
                                        ...
                                        <div className="p-6 sm:p-10 rounded-2xl sm:rounded-[40px] bg-primary/10 border border-primary/20 space-y-4 sm:space-y-6 shadow-sm">
                                            <p className="text-xs sm:text-sm font-semibold text-foreground/80 leading-relaxed uppercase tracking-tight">
                                                "{impactData.incident.aiRca}"
                                            </p>
                                            <div className="pt-2 sm:pt-4 flex flex-wrap items-center gap-2 sm:gap-3">
                                                 <div className="px-2.5 py-1 rounded-lg bg-primary/10 text-[8px] sm:text-[9px] font-bold text-primary tracking-widest uppercase border border-primary/20">Confidence: 98.4%</div>
                                                 <div className="px-2.5 py-1 rounded-lg bg-secondary text-[8px] sm:text-[9px] font-bold text-muted-foreground/40 tracking-widest uppercase border border-border">System Engine: V2.4</div>
                                            </div>
                                        </div>
                                    </div>
                                )}
                            ...
                            <div className="p-6 sm:p-12 bg-secondary/30 border-t border-border flex flex-col sm:flex-row gap-3 sm:gap-6">
                                <Button className="flex-1 rounded-xl sm:rounded-2xl text-[10px] sm:text-[11px] font-bold uppercase tracking-widest h-14 sm:h-16 bg-red-600 hover:bg-red-700 text-white shadow-lg sm:shadow-[0_15px_30px_rgba(220,38,38,0.3)] border-0 transition-all">
