        ...
        <div className="space-y-8 sm:space-y-12 pb-12 sm:pb-20 font-sans">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 sm:gap-12">
                    <div className="space-y-2 sm:space-y-3 lg:max-w-md">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-[10px] font-bold uppercase tracking-[.4em] text-muted-foreground/60">Endpoint Management</h2>
                        </div>
                    ...
                    <div className="flex-1 max-w-2xl">
                        <form onSubmit={handleAddMonitor} className="bg-card p-4 sm:p-7 rounded-[24px] sm:rounded-[40px] border border-border shadow-[0_20px_60px_-20px_rgba(0,0,0,0.05)] space-y-4 sm:space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                <div className="space-y-2">
                                    <InputField
                                        ...
                                        className="bg-secondary/20 border-border h-14 text-xs font-semibold rounded-2xl focus-visible:ring-4 focus-visible:ring-primary/5 focus-visible:border-primary/40 transition-all font-sans text-foreground placeholder:text-muted-foreground/30"
                                    />
                                </div>
                                <div className="space-y-2">
                                    <InputField
                                        ...
                                        className="bg-secondary/20 border-border h-14 text-xs font-semibold rounded-2xl focus-visible:ring-4 focus-visible:ring-primary/5 focus-visible:border-primary/40 transition-all text-foreground placeholder:text-muted-foreground/30"
                                    />
                                </div>
                            </div>
                            ...
                            {showGit && (
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 animate-in fade-in slide-in-from-top-4 duration-500 pt-2 border-t border-border">
                                    <div className="space-y-2">
                                        <InputField
                                        id="repoOwner"
                                        label="Org" 
                                            ...
                                            className="bg-secondary/20 border-border h-11 text-[10px] font-bold rounded-xl text-foreground placeholder:text-muted-foreground/30"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <InputField
                            ...
                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pt-4 border-t border-border">
                                <Button 
                                    ...
                                    className={`text-[9px] font-bold uppercase tracking-[.25em] gap-3 h-11 sm:h-12 px-5 rounded-2xl transition-all ${showGit ? 'bg-primary/10 text-primary' : 'text-muted-foreground/40 hover:bg-secondary'}`}
                                >
                ...
                {isLoading ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
                        {[1, 2, 3, 4].map(i => <div key={i} className="h-72 rounded-[24px] sm:rounded-[40px] bg-card border border-border animate-pulse" />)}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
                        ...
                        {monitors?.length === 0 && (
                            <div className="lg:col-span-2 h-[300px] sm:h-[450px] flex flex-col items-center justify-center border-2 border-dashed border-border rounded-[24px] sm:rounded-[50px] bg-card group hover:border-primary/20 transition-all gap-6 sm:gap-8 px-4">
                                ...
                                <div className="text-center space-y-3">
                                    <p className="text-[11px] font-bold uppercase tracking-[.5em] text-muted-foreground/40">Station Idle</p>
                                    <p className="text-xs font-semibold text-muted-foreground/20 uppercase tracking-widest">Awaiting First Endpoint Deployment</p>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className="space-y-6 sm:space-y-10">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 sm:gap-0">
                    <div className="flex gap-4 sm:gap-6 items-center">
                        <div className={`h-12 w-12 sm:h-16 sm:w-16 rounded-xl sm:rounded-2xl flex items-center justify-center border transition-all duration-500 shrink-0 ${isUp ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : isDegraded ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
                        ...
                        <div className="space-y-1.5">
                                <h3 className="text-base sm:text-xl font-bold uppercase tracking-tighter text-foreground group-hover:text-primary transition-colors flex items-center gap-2 sm:gap-3">
                                    <span className="truncate">{monitor.name}</span>
                                {hasGit && <Github className="h-3.5 w-3.5 text-muted-foreground/30" />}
                            </h3>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{monitor.url.replace('https://', '')}</p>
                        </div>
                    </div>
                    
                    <div className="text-right flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-3">
                        <div className={`px-3 sm:px-4 py-1.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-bold uppercase tracking-[.15em] sm:tracking-[.25em] border ${
                            monitor.status === 'GOOD' || monitor.status === 'UP' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' :
                            monitor.status === 'OK' ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' :
                            isDegraded ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' :
                        ...
                        {monitor.successKeyword && (
                            <div className="flex items-center gap-2 group/tip">
                                ...
                                <span className="text-[8px] font-bold text-muted-foreground/40 uppercase tracking-tighter">Content Verified</span>
                            </div>
                        )}
                    </div>
                </div>

                <div className="grid grid-cols-3 gap-3 sm:gap-8 pt-4 sm:pt-8 border-t border-border">
                    <div className="space-y-1 sm:space-y-1.5 text-center px-1 sm:px-4">
                        <p className="text-[10px] font-bold uppercase tracking-[.25em] text-muted-foreground">P95 Latency</p>
                    ...
                    <div className="space-y-1 sm:space-y-1.5 text-center px-1 sm:px-4 border-x border-border">
                        <p className="text-[10px] font-bold uppercase tracking-[.25em] text-muted-foreground">Score</p>
                        <p className="text-sm font-bold tracking-tight text-foreground tabular-nums">{monitor.healthScore || 100} PT</p>
                    </div>
                    <div className="space-y-1 sm:space-y-1.5 text-center px-1 sm:px-4">
                        <p className="text-[10px] font-bold uppercase tracking-[.25em] text-muted-foreground">Signals</p>
                        <p className="text-sm font-bold tracking-tight text-foreground tabular-nums">{logs?.totalSamples || 0} <span className="text-[9px] opacity-30 uppercase">Pulse</span></p>
                    </div>
                </div>

                <div className="h-20 sm:h-28 w-full opacity-60 group-hover:opacity-100 transition-all duration-700 bg-secondary/10 rounded-[16px] sm:rounded-[24px] p-1 sm:p-2">
                ...
                <div className="pt-4 sm:pt-6 border-t border-border flex items-center justify-between">
                    <Link to={`/dashboard/monitors/${monitor._id}`} className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-muted-foreground hover:text-primary transition-all flex items-center gap-2 px-3 sm:px-4 py-2 bg-secondary/10 hover:bg-primary/5 rounded-xl sm:rounded-2xl border border-transparent hover:border-primary/20">
                         Detailed Analytics
                    ...
                    <div className="flex gap-2">
                         ...
                         <Dialog open={isOpen} onOpenChange={setIsOpen}>
                            <DialogTrigger asChild>
                                <Button variant="ghost" size="icon" className="h-11 w-11 text-muted-foreground hover:text-primary hover:bg-primary/5 rounded-2xl transition-all border border-border/50 hover:border-primary/30 group/btn shadow-sm bg-card">
                                    <Pencil className="h-4.5 w-4.5 group-hover/btn:scale-110 transition-transform" />
                                </Button>
                            </DialogTrigger>
                            <DialogContent className="w-[95%] sm:max-w-[520px] bg-card rounded-[32px] sm:rounded-[40px] border border-border shadow-2xl p-0 outline-none overflow-hidden text-foreground">
                                <div className="p-6 sm:p-10 text-foreground">
                                    <DialogHeader className="space-y-4">
                                    <DialogTitle className="text-2xl font-bold uppercase tracking-tighter text-foreground flex items-center gap-4 outline-none">
                                        <div className="h-10 w-10 rounded-xl bg-primary/5 flex items-center justify-center">
                                            <Settings2 className="h-5 w-5 text-primary" />
                                        </div>
                                        Manage <span className="text-primary">Monitor</span>
                                    </DialogTitle>
                                    <DialogDescription className="text-xs font-semibold text-muted-foreground uppercase tracking-widest leading-relaxed">Modify endpoint credentials or content verification rules.</DialogDescription>
                                </DialogHeader>
                                <div className="py-6 sm:py-8 space-y-6 max-h-[60vh] sm:max-h-none overflow-y-auto px-1 scrollbar-hide">
                                    <div className="space-y-2">
                                        <label htmlFor={`editName-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Friendly Name</label>
                                        <InputField 
                                            id={`editName-${monitor._id}`}
                                            name="name"
                                            value={editName} 
                                            onChange={(e) => setEditName(e.target.value)} 
                                            className="bg-secondary/20 border-border h-14 text-xs font-semibold rounded-2xl text-foreground placeholder:text-muted-foreground/30" 
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label htmlFor={`editUrl-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Endpoint URL</label>
                                        <InputField 
                                    ...
                                    <div className="space-y-2">
                                        <label htmlFor={`editKeyword-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Success Keyword</label>
                                        <InputField 
                                    ...
                                    <div className="space-y-2">
                                        <label htmlFor={`editApiUrl-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Backend Health URL</label>
                                        <InputField 
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <InputField 
                                            ...
                                            className="bg-secondary/20 border-border h-14 text-xs font-semibold rounded-2xl text-foreground placeholder:text-muted-foreground/30" 
                                        />
                                        <p className="text-[9px] font-bold text-primary uppercase tracking-widest mt-2 px-1 italic">* Autonomous Hazard Detection: Detects outages even on 200 OK</p>
                                    </div>

                                    {/* Autonomous Logic Section */}
                                    <div className="pt-6 border-t border-border space-y-6">
                                        <div className="flex items-center gap-3">
                                            <div className="h-5 w-5 rounded-lg bg-foreground text-card flex items-center justify-center">
                                                <Github className="h-3 w-3" />
                                            </div>
                                            <h4 className="text-[10px] font-bold uppercase tracking-widest text-foreground">Autonomous Logic Binding</h4>
                                        </div>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div className="space-y-2">
                                                <label htmlFor={`githubOwner-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">GitHub Owner</label>
                                                <InputField 
                                                    ...
                                                    className="bg-secondary/20 border-border h-14 text-xs font-semibold rounded-2xl text-foreground" 
                                                />
                                            </div>
                                            <div className="space-y-2">
                                                <label htmlFor={`githubRepo-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Repo Name</label>
                                                <InputField 
                                        ...
                                        <div className="space-y-2">
                                            <label htmlFor={`githubBranch-${monitor._id}`} className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Production Branch</label>
                                            <InputField 
                                ...
                                <DialogFooter className="p-6 sm:p-10 pt-0 sm:pt-0 gap-3">
                                    <Button onClick={() => setIsOpen(false)} variant="ghost" className="h-14 flex-1 rounded-2xl font-bold uppercase tracking-widest text-[10px] text-muted-foreground">Cancel</Button>
                         ...
                         <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-11 w-11 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded-2xl transition-all border border-border/50 hover:border-red-500/30 group/del shadow-sm bg-card" 
