...
const DashboardLayout = () => {
  useNotificationSync(); // 🛰️ Activate real-time status notifications
  const navigate = useNavigate();
  const location = useLocation();
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col lg:pl-72 relative">
            <header className={`h-16 sm:h-20 flex items-center justify-between px-3 sm:px-6 lg:px-8 border-b border-border bg-background/80 backdrop-blur-xl sticky top-0 z-40 transition-all`}>
                <div className="flex items-center gap-2 sm:gap-6 flex-1 min-w-0">
                    <Sheet open={isSidebarOpen} onOpenChange={setIsSidebarOpen}>
                        <SheetTrigger asChild>
                            <Button variant="ghost" size="icon" className="lg:hidden h-8 w-8 sm:h-11 sm:w-11 border border-border rounded-lg sm:rounded-2xl bg-card shrink-0">
