...
const SidebarContent = ({ onNavItemClick }: { onNavItemClick?: () => void }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-slate-100 relative">
        <div className="absolute inset-0 bg-slate-50 opacity-[0.2] pointer-events-none" />
        
        <div className="h-20 flex items-center px-8 shrink-0 relative border-b border-slate-50">
        ...
        <div className="px-5 mb-8">
            <div className="inline-flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-indigo-50/50 border border-indigo-100 shadow-sm relative overflow-hidden group/control">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.15em] text-indigo-600/80 leading-none relative">Workspace Control</span>
            </div>
        </div>
            {navItems.map((item) => {
                ...
                return (
                    <Link
                        ...
                        className={`flex items-center gap-3.5 px-4 py-3.5 rounded-2xl transition-all group relative border ${
                            active 
                            ? "bg-slate-50 text-primary border-slate-100 shadow-sm" 
        ...
        <div className="p-5 relative border-t border-slate-50">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 space-y-3">
                <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-xl bg-white border border-slate-100 text-primary shadow-sm flex items-center justify-center text-xs font-bold italic">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 hover:text-red-500 hover:bg-red-50" onClick={handleLogout}>
...
const DashboardLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 selection:bg-primary/10 font-sans overflow-hidden">
        {/* Sidebar - Desktop */}
        <aside className="hidden lg:flex w-[280px] flex-col shrink-0 z-50">
            <SidebarContent />
        </aside>
        ...
        <div className="flex-1 flex flex-col min-w-0 relative">
            ...
            <header className={`h-16 sm:h-20 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 z-40 relative transition-all duration-300 ${
                ...
                <div className="flex items-center gap-6 flex-1">
                    <Sheet>
                        <SheetTrigger asChild>
                            <Button variant="ghost" size="icon" className="lg:hidden h-10 w-10 border border-slate-200 rounded-xl bg-white">
                                <Menu className="h-5 w-5" />
                            </Button>
                        </SheetTrigger>
                        <SheetContent side="left" className="p-0 border-r border-slate-100 w-72">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="hidden md:flex items-center gap-3 bg-white border border-slate-200 rounded-2xl px-5 py-2.5 w-full max-w-sm focus-within:border-primary/40 focus-within:ring-4 focus-within:ring-primary/5 transition-all group">
                            <Search className="h-4 w-4 text-slate-300 group-focus-within:text-primary transition-all" />
                            <label htmlFor="global-search" className="sr-only">Search Workspace</label>
                            <Input
                                id="global-search"
                                name="search"
                                placeholder="Search Workspace..."
                                className="bg-transparent border-0 h-6 text-xs font-semibold placeholder:text-slate-300 focus-visible:ring-0 p-0"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="hidden lg:flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-2xl text-[10px] font-bold text-slate-500 uppercase tracking-widest shadow-sm">
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="relative h-11 w-11 rounded-2xl bg-white border border-slate-200 hover:bg-slate-50 transition-all group shadow-sm">
                                <Bell className="h-4.5 w-4.5 text-slate-400 group-hover:text-primary transition-colors" />
                                <span className="absolute top-3.5 right-3.5 h-2 w-2 rounded-full bg-red-500 border-2 border-white" />
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-[380px] bg-white border-slate-200 p-3 shadow-2xl rounded-3xl mt-2 animate-in slide-in-from-top-2">
                             <DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-widest text-slate-400 px-3 py-2">Operational Alerts</DropdownMenuLabel>
                                ...
                                <div className="p-4 hover:bg-slate-50 rounded-2xl transition-colors cursor-pointer group border border-transparent hover:border-slate-100">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-xs font-bold text-slate-900">Critical: Endpoint Timeout</p>
                                            <p className="text-[10px] font-medium text-slate-400 mt-1">Monitor US-East-1 has exceeded 5s response time.</p>
                                        </div>
                                    </div>
                                </div>
                             </div>
                    ...
                    <Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl bg-white border border-slate-200 shadow-sm text-slate-400 hover:text-primary transition-colors" onClick={() => navigate('/dashboard/settings')}>
